...
/**
 * The Frameworks & Drivers implementation of ItemIconGateway, calling GGG's
 * public Item Icons static-data endpoint (docs/DATA_SOURCES.md § Item
 * Icons). Fetches and caches the full catalog once for the app's lifetime --
 * it's static, rarely-changing data, and resolving hundreds of currencies
 * must never cost a network call each.
 *
...
public class GggItemIconGateway implements ItemIconGateway {
    ...
    private synchronized void ensureCatalogLoaded() {
        ...
        String json = restClient.get().uri("/api/trade/data/static").retrieve().body(String.class);
        normalize(json);
    }

...
Buy 210 Scrolls of Wisdom for 1 Chaos Orb on the Exchange → vendor recipe (3:1) → 70 Portal Scrolls → vendor recipe (7:1) → 10 Orbs of Transmutation → sell the Orbs of Transmutation on the Exchange for ~2 Chaos Orbs. Net: ~2x on the original Chaos Orb.
(Corrected 2026-08-09 — the original version of this example ran the vendor step backwards, converting a higher-tier orb into more scrolls; no such recipe exists. Vendor recipes only ever convert a cheaper item into a pricier one, per the verified chain below.)

**Requirements:**
- List all currency → vendor-item conversion paths using vendor sell rates and known vendor recipes, sourced and verified from the [PoE Wiki Currency page § Vendor prices chart](https://www.poewiki.net/wiki/Currency#Vendor_prices_chart) (the upgrade chain) and the [PoE Wiki Vendor recipe system page § Downgrade](https://www.poewiki.net/wiki/Vendor_recipe_system#Downgrade) (the reverse, "at a sharp penalty") — captured into `app/priv/reference-data/vendor-recipes.json` (see [DATA_SOURCES.md § Vendor Sell Rates & Recipes](DATA_SOURCES.md#vendor-sell-rates--recipes) for the capture methodology and verified last-checked date).
- **In scope is exactly the currency-in/currency-out recipes across both tables** (e.g. Scroll of Wisdom → Portal Scroll → Orb of Transmutation → … up through Orb of Alchemy on the upgrade side; Orb of Transmutation → 4x Scroll of Wisdom on the downgrade side) — since the Exchange only trades currency items, only a recipe whose *input* is itself a currency item is chainable from an Exchange purchase. **Shards and scroll fragments are out of scope**: they combine into full orbs automatically by stacking in the inventory, not via an NPC vendor sale, so they don't fit the "sold to a vendor" model this feature chains through. **Two Downgrade recipes needing multiple distinct input or output currencies at once (Jeweller's Orb, Mirror of Kalandra) are also out of scope** — `VendorRecipe` only models a single input currency and a single output currency per row (see [DATA_SOURCES.md](DATA_SOURCES.md#vendor-sell-rates--recipes)).
...
- **Chains are found by graph search over the captured recipes (input currency → output currency, keyed by display name), capped at 3 consecutive vendor-recipe conversions.** This bounds the search over the recipe set and keeps the rendered step-by-step path readable; a theoretical longer chain is not computed. **A chain may never revisit a currency it has already passed through, including looping back to its own starting currency** — such a path isn't a real flip (you'd just be holding what you started with, at a loss after two conversion steps). **Multiple distinct chains between the same start and end currency (different intermediate hops) are kept as separate opportunities**, not deduplicated to "the best one" — consistent with how every other technique keeps distinct routes separate.
- **Pricing convention matches Bulk Buy (§ 7.5) and Divination Card (§ 7.3): the entry leg (buying the chain's starting currency) is priced competitively (the undercut suggestion, § 7.2's convention); the exit leg (reselling the chain's ending currency) is priced at the plain market rate** — a one-directional dump into existing demand, not a round-trip order, same reasoning as those two features' own resale legs. **Both legs independently prefer a Chaos-side market, falling back to Divine** — the two legs are unrelated trades, same as Divination Card's buy/resale legs. **Both legs also re-orient if the "other" side is worth more than 1 base unit**, the same auto-detected per-leg fallback § 7.2/7.3 already use.
...
- **One checkbox per technique, scoped to the active tab** — Grand Exchange Flip shows three checkboxes (Exchange Spread, Divination Card, Bulk Buy), Vendor Flip shows one (Vendor Recipe). Switching tabs doesn't change a technique's remembered checked/unchecked state, only which techniques are visible to toggle and which rows the table shows at all.
...
- **Persisted client-side (browser storage), surviving a full page reload** — same mechanism and per-browser scope as Favorites (§7.10): a personal preference, not synced across devices or sessions elsewhere. This includes the active tab itself, not just the per-technique checkboxes. A first-ever visit (nothing saved yet) falls back to the all-checked, Grand-Exchange-Flip-active default above.

...
- **Requires a descriptive `User-Agent` header when called from a datacenter/cloud IP.** Confirmed against production 2026-08-06: a request with Java's default `User-Agent` returned `HTTP 403 {"error":{"code":6,"message":"Forbidden"}}` when made from Render's IP range, though the identical call worked fine from a residential IP during local testing. `api.pathofexile.com` (League List) and `web.poecdn.com` (Currency Exchange) were unaffected — only this `www.pathofexile.com`-hosted endpoint blocks it, consistent with the main site carrying stricter bot protection than the dedicated API/CDN subdomains. Fix: set a descriptive `User-Agent` identifying the app (e.g. `poe-currency-flip/0.1 (+<repo URL>)`) on every request to any `pathofexile.com`/`poecdn.com` endpoint, not just this one — applied centrally via a `RestClientCustomizer` (`GatewayConfig`) rather than per-gateway.

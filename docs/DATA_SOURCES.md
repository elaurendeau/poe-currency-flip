...
**Source:** `GET https://web.poecdn.com/api/trade/data/static` (or the identical `www.pathofexile.com` path) -- **not called live in production**, see IP-block note below. Vendored into `backend/src/main/resources/reference-data/item-icons-catalog.json` and read from the classpath instead.
**Auth:** None required.
...
- **Fix: stopped calling this endpoint live in production.** Since this catalog is static, rarely-changing data (new entries only appear with new leagues/items), it's captured once into `backend/src/main/resources/reference-data/item-icons-catalog.json` and read from the classpath — the same treatment already given to the vendor-recipe data below. **Refresh procedure** (needed only when GGG adds new items GGG-side and a currency starts resolving as unknown): from a residential/non-datacenter IP, `curl https://web.poecdn.com/api/trade/data/static -o backend/src/main/resources/reference-data/item-icons-catalog.json`, then commit. `GggItemIconGateway` takes no HTTP client dependency at all now.

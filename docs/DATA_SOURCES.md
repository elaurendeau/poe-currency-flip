...
**Endpoint:** `GET https://web.poecdn.com/api/trade/data/static` (CDN mirror -- see IP-block note below; identical JSON to the `www.pathofexile.com` path of the same name)
**Auth:** None required.
...
- **`www.pathofexile.com` blocks Render's IP range outright — not a `User-Agent` issue.** Confirmed against production 2026-08-06: every request to `www.pathofexile.com/api/trade/data/static` from Render returned `HTTP 403 {"error":{"code":6,"message":"Forbidden"}}`. Initially assumed to be a missing/generic `User-Agent`, but disproved by direct testing: from a residential IP, *any* non-empty `User-Agent` (including Java's own default) succeeds — only a completely blank one 403s. The block is therefore IP-reputation-based at Cloudflare's edge (a documented, known issue for third-party PoE tools hosted on cloud/datacenter IPs), not something a request header can fix. **Fix: fetch this endpoint from `web.poecdn.com` instead** (the same CDN host `Currency Exchange` already uses successfully from Render) — it serves byte-identical JSON at the same path, and is not behind the same bot-protection rules as the main site. Still set a descriptive `User-Agent` on all GGG requests regardless (`GatewayConfig`'s `RestClientCustomizer`) as standard API-citizenship practice, but do not rely on it as the fix for this specific block.
- **Icon image URLs still resolve against `www.pathofexile.com`, not the CDN mirror.** The `image` field in the response is a relative path (e.g. `/gen/image/.../CurrencyPortal.png`); prefixing it with `https://www.pathofexile.com` is correct and unaffected by the block above, since those URLs are loaded by the end user's own browser (a residential IP), never fetched by this backend.
